import { DELETE_CROP_MUTATION } from '@/contexts/crops/infrastructure/graphql/mutations/crops-mutations.graphql';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.mutate({
      mutation: DELETE_CROP_MUTATION,
      fetchPolicy: 'network-only',
      variables,
    });

    const nextRes = NextResponse.json(response.data.deleteCrop);
