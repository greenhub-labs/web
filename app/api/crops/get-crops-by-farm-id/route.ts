import { GET_CROPS_BY_FARM_ID_QUERY } from '@/contexts/crops/infrastructure/graphql/queries/crops-queries.graphql';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.query({
      query: GET_CROPS_BY_FARM_ID_QUERY,
      fetchPolicy: 'network-only',
      variables,
    });

    const nextRes = NextResponse.json(response.data.getCropsByFarmId);
