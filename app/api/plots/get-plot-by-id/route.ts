import { GET_PLOT_BY_ID_QUERY } from '@/contexts/plots/infrastructure/graphql/queries/plots-queries.graphql';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.query({
      query: GET_PLOT_BY_ID_QUERY,
      fetchPolicy: 'network-only',
      variables,
    });

    const nextRes = NextResponse.json(response.data.getPlotById);
