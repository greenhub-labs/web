import { UPDATE_PLOT_MUTATION } from '@/contexts/plots/infrastructure/graphql/mutations/plots-mutations.graphql';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.mutate({
      mutation: UPDATE_PLOT_MUTATION,
      fetchPolicy: 'network-only',
      variables,
    });

    const nextRes = NextResponse.json(response.data.updatePlot);
